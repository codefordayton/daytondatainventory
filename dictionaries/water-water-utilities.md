...
- **Service URL:** https://maps.daytonohio.gov/gisservices/rest/services/Water/Water_Utilities/MapServer
- **Item page:** https://daytonohio.maps.arcgis.com/home/item.html?id=Water_Water_Utilities
...
## Layer 4: Water Control Valve Labels
...
## Layer 5: Water Curb Stop Valves
...
## Layer 7: Water System Valve Labels
...
## Layer 10: Water Network Structures
...
| `COMPTYPE` | Integer |  |  |
| `SOURCE` | String | Data Source | **Values:** `GUS` = DATA PLACED VIA GUS; `PRPLAT` = PROPOSED PLAT; `PRCONS` = PROPOSED CONSTRUCTION; `SRVY` = SURVEY ASBUILT; `NSRVY` = NON SURVEY ASBUILT; `DSPCH` = DISPATCH CALL IN; `DAYTON` = CITY OF DAYTON; `MCSED` = MONTGOMERY COUNTY SANITARY; `GISD` = NPDES STORM OUTFALL GPS COLLECTION PROJECT (GIS DYNAMICS); `STONE` = STONE ENVIRONMENTAL GPS; `SMPPII` = STORM MASTER PLAN PHASE II; `SMPPIIEST` = STORM MASTER PLAN PHASE II - ESTIMATED; …(+2 more) · len 10 |
| `VERSIONNAME` | String | VersionName | len 50 |
| `EDITTOOL` | String | EditTool | len 50 |
| `EDITTASK` | String | EditTask | len 50 |
| `ADDRKEY` | Integer | Address |  |
| `ADDRQUAL` | String | Address Info | len 254 |
...
## Layer 12: Water Service Connections
...
| `last_edited_user` | String |  | len 255 |
| `last_edited_date` | Date |  |  |
| `PITCHERGIVEN` | String | Pitcher Given | **Values:** `Y` = Yes; `N` = No; `U` = Unknown; `NA` = NA; `Other` = Other; `Unsure` = Unsure; `Yes` = Yes; `No` = No · len 50 |
...
## Layer 14: Water Lateral Line Label
...
## Layer 17: Water Operational Areas
